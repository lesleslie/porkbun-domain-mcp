"""Tool profile registration groups for porkbun-domain-mcp MCP server.
...
``PORKBUN_DOMAIN_TOOL_PROFILE`` environment variable.
...
Profile tiers (Tier-A trivial — 3-tier mapping per the W4 plan):
    ...
    MINIMAL:  Only the MCP ``health_check`` tool + the ``discover_tools``
              ...
              deployments where the 5 PorkbunClient-bound tools would
              fail (no credentials, no Porkbun instance reachable).
    STANDARD: All 5 porkbun-domain-mcp tools + ``health_check`` +
              ``discover_tools`` (Tier-A trivial: same set as FULL —
              there's no "core subset" to drop at STANDARD).
    FULL:     All 5 porkbun-domain-mcp tools + ``health_check`` +
              ``discover_tools`` (registered via the
              ``register_all_fn`` bulk registration path).

The dispatch surface (``PROFILE_REGISTRATIONS`` + ``_GROUP_REGISTRY`` +
``register_all_tool_groups`` + ``apply_porkbun_domain_tool_profile``) is
consumed by ``porkbun_domain_mcp.server.create_app`` which delegates to
...
# attribute name on the ``porkbun_domain_mcp.tools`` package. THIS is the
...
# ``getattr(porkbun_domain_mcp.tools, attr_name)`` — no name-specific
...
def _build_registration_map(
    settings: PorkbunDomainSettings, client: PorkbunDomainClient
    ...
    ``porkbun_domain_mcp.tools`` package (no hard-coded name-specific
    ...
    capture of ``settings`` and ``client`` (the W3.1 graphics-mcp
    lesson + the W3.3 ``_register_crs_with_app`` pattern).

    Args:
        settings: The caller-supplied ``PorkbunDomainSettings`` instance to
        ...
        client: The pre-constructed ``PorkbunDomainClient`` instance. Same
    ...
    Returns:
        Mapping from group key (e.g. ``"domain_tools"``) to a single-arg
        async-or-sync callable that takes the ``FastMCP`` server.
    """
    from porkbun_domain_mcp import tools as _tools_module
...
def register_all_tool_groups(
    server: FastMCP, settings: PorkbunDomainSettings, client: PorkbunDomainClient
) -> None:
    """Bulk register every porkbun-domain-mcp tool group (called at FULL profile).

    Iterates ``_GROUP_REGISTRY`` directly — no name-specific
    conditionals. Each entry's ``attr_name`` is looked up dynamically on
    the ``porkbun_domain_mcp.tools`` package. Used as ``register_all_fn``
    for the W0 helper.

    Args:
        server: FastMCP server instance.
        settings: The caller-supplied ``PorkbunDomainSettings`` to pass
            through to every group registration (NOT re-loaded from env).
        client: The pre-constructed ``PorkbunDomainClient`` to bind into
...
async def apply_porkbun_domain_tool_profile(
    server: FastMCP, settings: PorkbunDomainSettings, client: PorkbunDomainClient
) -> None:
    """Apply the ``PORKBUN_DOMAIN_TOOL_PROFILE`` dispatch to ``server`` at startup.

    Async because the W0 helper is async; called from
    ``porkbun_domain_mcp.server.create_app`` via
    ...
    await _apply_tool_profile(
        server,
        profile_env_var="PORKBUN_DOMAIN_TOOL_PROFILE",
